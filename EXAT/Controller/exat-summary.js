		                    ...
		                    htmlTable2+="<thead>";
		                        htmlTable2+="<tr >";
		                           ...
		                           // htmlTable2+="<th >เพิ่ม/ลดเทียบปีก่อนหน้า</th>";
		                            htmlTable2+="<th >%เพิมลดจากปีที่แล้ว</th>";
				...
				$.ajax({
					...
					success:function(data){
		                ...
		                $("#Chart_exat_summary_by_traffic_accumulate_table").html(htmlTable2);
							...
							$("#Chart_exat_summary_by_traffic_accumulate_table").html("<table class=\"table table-bordered\"><tbody><tr><td colspan=\"2\"><center>ปีงบประมาณที่เลือก \"ยังไม่มีข้อมูล\"</center></td></tr></tbody><thead><tr><th>ปริมาณจราจรสะสม (ล้านเที่ยว)</th><th>ปริมาณจราจรเฉลี่ยต่อวัน (ล้านเที่ยว)</th> </tr></thead></table>");
							//$("#Chart_exat_traffic_month_by_traffic_accumulate_table").text('ปีงบประมาณที่เลือก "ยังไม่มีข้อมูล"');
						}
				...
				$.ajax({
					...
					success:function(data){
						...
						if(data != ""){
							...
							$("#Chart_exat_summary_by_traffic_highway").css({"margin-top":"40px","margin-bottom":"120px" });
							option=[];
				...
				$.ajax({
					...
					success:function(data){
		                ...
		                $("#Chart_exat_summary_by_sales_accumulate_table").html(htmlTable2);
							...
							$("#Chart_exat_summary_by_sales_accumulate_table").html("<table class=\"table table-bordered\"><tbody><tr><td colspan=\"2\"><center>ปีงบประมาณที่เลือก \"ยังไม่มีข้อมูล\"</center></td></tr></tbody><thead><tr><th>ยอดจำหน่ายสะสม (บัตร)</th><th>ยอดจำหน่ายเฉลี่ยต่อวัน (บัตร)</th> </tr></thead></table>");
							//$("#Chart_exat_traffic_month_by_traffic_accumulate_table").text('ปีงบประมาณที่เลือก "ยังไม่มีข้อมูล"');
						}
				...
				$.ajax({
					...
					success:function(data){
		                ...
		                $("#Chart_exat_summary_by_topup_accumulate_table").html(htmlTable2);
							...
							$("#Chart_exat_summary_by_topup_accumulate_table").html("<table class=\"table table-bordered\"><tbody><tr><td colspan=\"2\"><center>ปีงบประมาณที่เลือก \"ยังไม่มีข้อมูล\"</center></td></tr></tbody><thead><tr><th>ปริมาณการเติมเงิน (ครั้ง)</th><th>ปริมาณการเติมเงินเฉลี่ยต่อวัน (ครั้ง)</th> </tr></thead></table>");
							//$("#Chart_exat_traffic_month_by_traffic_accumulate_table").text('ปีงบประมาณที่เลือก "ยังไม่มีข้อมูล"');
						}
